...
class OrdemCompraRepository {
  async findByPedidoId(pedidoId, conn = db) {
    const [rows] = await conn.query('SELECT * FROM ordens_compra WHERE pedido_id = ?', [pedidoId]);
    return rows[0] || null;
  }
  ...
  async create(conn, data) {
    const [result] = await conn.query(
      `INSERT INTO ordens_compra (
        pedido_id, fornecedor_id, fornecedor_nome, fornecedor_endereco, fornecedor_telefone,
